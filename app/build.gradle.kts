...
tasks.register("copyApkToRoot") {
    val src = File(projectDirFile, "build/outputs/apk/debug/app-debug.apk")
    val dest = File(rootDirFile, "USANA_Insight_Hub.apk")
    doLast {
        if (src.exists()) {
            src.copyTo(dest, overwrite = true)
            println("APK copied successfully to project root: ${dest.absolutePath}")
        } else {
            println("Error: Source APK does not exist at ${src.absolutePath}")
        }
    }
}
